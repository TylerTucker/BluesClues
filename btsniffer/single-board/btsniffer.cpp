...
// TYLER added additional libraries
#include <zmq.hpp>
					...
					case LAP_STATE_BRUTE_FORCING:
						{
							...
							if (llabs(lap_map[_lap].get_ts() - timenow_sec_us) < DELTA_TS_SAME_THRESHOLD) {
								...
								lap_map[_lap].set_ts(timenow_sec_us);
							// TYLER needs to add ZMQ message for updated UAP list
							// const char* header = "LAP";
							// size_t size = strlen(header);
							// zmq::message_t message(size);
							// std::memcpy (message.data(), header, size);
							zmq::message_t message("LAP", 3);
							socket.send(message, ZMQ_NOBLOCK);
							socket.send(&_lap,	sizeof(_lap), ZMQ_NOBLOCK);
							socket.send(&confirmed_uap, sizeof(confirmed_uap), ZMQ_NOBLOCK);
							lap_node::uap_data* uaps = lap_map[_lap].uaps;
							// uint32_t my_uap = uaps[0].uap;
							// socket.send(&my_uap, sizeof(uint32_t), ZMQ_NOBLOCK);
							for (int mi = 0; mi < 32; mi++) {
								uint32_t my_uap = uaps[mi].uap;
								if (my_uap != 0)
									socket.send(&my_uap, sizeof(uint32_t),ZMQ_NOBLOCK);
							}
								continue;
								...
								// TYLER sends UAPs to Ubertooth
								zmq::message_t message("LAP", 3);
								socket.send(message, ZMQ_NOBLOCK);
								socket.send(&_lap,	sizeof(_lap), ZMQ_NOBLOCK);
								uint8_t confirmed_uap = 2;
								socket.send(&confirmed_uap, sizeof(confirmed_uap), ZMQ_NOBLOCK);
								lap_node::uap_data* uaps = lap_map[_lap].uaps;
								// uint32_t my_uap = uaps[0].uap;
								// socket.send(&my_uap, sizeof(uint32_t), ZMQ_NOBLOCK);
								socket.send(&uap_found[0], sizeof(uint32_t), ZMQ_NOBLOCK);
								socket.send(&uap_found[1], sizeof(uint32_t), ZMQ_NOBLOCK);
								// for (int mi = 0; mi < 32; mi++) {
								// 	uint32_t my_uap = uaps[mi].uap;
								// 	if (my_uap != 0)
								// 		socket.send(&my_uap, sizeof(uint32_t),ZMQ_NOBLOCK);
								...
								lap_map.erase(_lap);
									...
									std::cout << boost::format("%d possible UAPs remaining") % count_valid_uap << std::endl;
								}
