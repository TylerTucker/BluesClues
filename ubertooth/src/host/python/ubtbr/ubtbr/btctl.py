...
STATE_TO_STR = {
	...
	BTCTL_STATE_FOUND:	"FOUND",
	BTCTL_STATE_WRONG:	"WRONG",
	BTCTL_STATE_QUIET:	"QUIET",
...
class BTCtlPagingCmd(BTCtlCmd):
	def __init__(self, bt, bdaddr):
		super().__init__(bt)
		self._bdaddr = bdaddr
		self._lmp = LMPMaster(self, addr=self._bdaddr)
		self.lt_addr = None
		self._l2cap_command_id = 0xc8
		# self.page_state = BTCTL_STATE_STANDBY
	...
	def _handle_state(self, state, reason):
		...
		if state == BTCTL_STATE_PAGE:
			...
			if reason == BTCTL_REASON_SUCCESS:
				...
				self._lmp.start()
				# TYLER
				# self.page_state = BTCTL_STATE_FOUND
				# self._done = True
				# self._ready = True
				# self.stop()
			else:
				log.info ("Connection status %d"%reason)
		elif self._lmp.success:
			self.page_state = BTCTL_STATE_FOUND
			self._done = True
			print('FOUND')
		elif state == BTCTL_STATE_FOUND:
			self.page_state = BTCTL_STATE_FOUND
			self._done = True
			print('FOUND')
			# self._ready = True
			self.stop()
		elif state == BTCTL_STATE_WRONG:
			self.page_state = BTCTL_STATE_WRONG
			self._done = True
			print('WRONG')
			self.stop()
		elif state == BTCTL_STATE_QUIET:
			self.page_state = BTCTL_STATE_QUIET
			self._done = True
			print('QUIET')
			self.stop()
		elif state == BTCTL_STATE_CONNECTED:
...
class BTCtl:
	...
	def _send_cmd(self, t, data=b''):
		# Pack header as little endian unsigned int
		hdr = pack("<I", t)
		# print(data)
	...
	def send_paging_cmd(self, bdaddr):
		# Pack BD_ADDR as little endian unsigned long long
